...
import { NodeMetrics, NodeCondition, NodeTaint } from '../types';
  ...
  async collect(): Promise<NodeMetrics[]> {
    try {
      ...
      for (const node of nodes) {
        ...
        const metrics = nodeMetrics.find((m: any) => m.metadata?.name === nodeName);
        ...
        // Parse conditions
        const conditions: NodeCondition[] = (node.status?.conditions || []).map((c) => ({
          type: c.type || '',
          status: c.status || '',
          reason: c.reason,
          message: c.message,
        ...
        // Parse taints
        const taints: NodeTaint[] | undefined = node.spec?.taints?.map((t) => ({
          key: t.key || '',
          value: t.value,
          effect: t.effect || '',
        }));
