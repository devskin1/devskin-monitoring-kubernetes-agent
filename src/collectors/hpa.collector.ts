import { KubeConfig, AutoscalingV2Api } from '@kubernetes/client-node';
import { HPAMetrics } from '../types';

export class HPACollector {
  private k8sApi: AutoscalingV2Api;
  private clusterName: string;

  constructor(kubeConfig: KubeConfig, clusterName: string) {
    this.k8sApi = kubeConfig.makeApiClient(AutoscalingV2Api);
    this.clusterName = clusterName;
  }

  async collect(): Promise<HPAMetrics[]> {
    try {
      const hpasResponse = await this.k8sApi.listHorizontalPodAutoscalerForAllNamespaces();
      const hpas = hpasResponse.body.items;

      const result: HPAMetrics[] = [];
      ...
      for (const hpa of hpas) {
        ...
        for (const metric of metricDetails) {
          if (
            ...
            health = 'warning';
            issues.push(
              `${metric.resource_name || metric.type} at ${metric.current_value}% (target: ${
                metric.target_value
              }%)`
            );
          }
