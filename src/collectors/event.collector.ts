import { KubeConfig, CoreV1Api, EventsV1Api } from '@kubernetes/client-node';
import { K8sEvent } from '../types';

export class EventCollector {
  private coreApi: CoreV1Api;
  private eventsApi: EventsV1Api;
  ...
  async collect(): Promise<K8sEvent[]> {
    try {
      ...
      const result: K8sEvent[] = [];
      const now = new Date();

      for (const event of events) {
        const eventTime = new Date(
          event.eventTime || event.lastTimestamp || event.metadata?.creationTimestamp || Date.now()
        );
  ...
  async collectCriticalEvents(): Promise<K8sEvent[]> {
